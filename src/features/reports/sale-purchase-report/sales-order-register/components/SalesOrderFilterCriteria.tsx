// app/sales-order-register/components/SalesOrderFilterCriteria.tsx
...
import { SORT_BY_OPTIONS, ORDER_STATUS_OPTIONS } from "../constants/salesOrderDefaults";
import { useBrands, useClasses, useSubClasses, useStates } from "../hooks/useSalesOrder";
...
const DEFAULT_SALES_ORDER_FILTER: Partial<SalesOrderFilterState> = {
    strbrand: '',
    strclass: '',
    strsubclass: '',
    sortby: 0,
...
interface SalesOrderFilterCriteriaProps {
    visible: boolean;
    onClose: () => void;
    onApply: (filters: Partial<SalesOrderFilterState>) => void;
    onClear: () => void;
    filterParams: SalesOrderFilterState;
    localFilters: Partial<SalesOrderFilterState>;
    setLocalFilters: React.Dispatch<React.SetStateAction<Partial<SalesOrderFilterState>>>;
    userId: number;
    companyId: number;
}

const SalesOrderFilterCriteria: React.FC<SalesOrderFilterCriteriaProps> = ({
    visible,
    onClose,
    onApply,
    onClear,
        ...
        if (localFilters.partyid && localFilters.partyid !== 0) {
            if (!selectedPartyName) {
                const fetchPartyName = async () => {
                    try {
                        const response = await fetch(`${process.env.NEXT_PUBLIC_PROJECT_API_ENDPOINT}/customer/${localFilters.partyid}?userid=${userId}&compid=${companyId}`);
    ...
    const handleTagBoxChange = useCallback(
        (value: number[], fieldName: keyof SalesOrderFilterState) => {
    ...
    const searchPartyColumns = [
        ...
        { key: "subledgertypenm", label: "Party Type" },
...
export default SalesOrderFilterCriteria;