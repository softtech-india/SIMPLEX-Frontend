...
import { ProdClassDataGrid } from './components/ProdClassDataGrid';
import { ProdClassForm } from './components/ProdClassForm';
import { useProdClasses, useProdClass } from './hooks/prodClass';
import { ProdClass, OperationMode } from './types/prodClass.types';
...
export default function ProdClassModule() {
  ...
  // State
  const [selectedRow, setselectedRow] = useState<ProdClass | null>(null);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [formMode, setFormMode] = useState<OperationMode>('Add');
  const [formProdClassId, setFormProdClassId] = useState(0);
  const { data: ProdClassList = [], isLoading, refetch } = useProdClasses();
  ...
  const openForm = useCallback((mode: OperationMode) => {
    ...
    if (mode === 'Add') {
      setFormProdClassId(0);
      setselectedRow(null);
    } else {
      setFormProdClassId(selectedRow?.id ?? 0);
    }
  ...
  const handleExport = useCallback(() => {
    if (!ProdClassList || ProdClassList.length === 0) return;

    // Generate columns dynamically from first row keys
    const columns: ExcelColumn[] = Object.keys(ProdClassList[0]).map((key) => ({
