...
import { Ledger, OperationMode } from './types/ledger.types';
...
export default function LedgerModule() {
  ...
  // State
  const [selectedRow, setselectedRow] = useState<Ledger | null>(null);
  ...
  const openForm = useCallback((mode: OperationMode) => {
    ...
    if (mode === 'Add') {
      ...
      setFormLedgerId(selectedRow?.id ?? 0);
    }
  ...
  const handleExport = useCallback(() => {
    if (!ledgerList || ledgerList.length === 0) return;

    // Generate columns dynamically from first row keys
    const columns: ExcelColumn[] = Object.keys(ledgerList[0]).map((key) => ({
